...
// ── Styles ────────────────────────────────────────────────────────────────────
const MODAL_INPUT = 'w-full px-3.5 py-2.5 rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 focus:border-[var(--brand-500)] focus:ring-4 focus:ring-[var(--brand-100)] dark:focus:ring-[var(--brand-900)]/30 outline-none text-[13px] text-slate-900 dark:text-slate-100 font-mono leading-6 resize-none placeholder:text-slate-400 dark:placeholder:text-slate-500 dark:text-slate-400 dark:text-slate-500';
const FIELD_INPUT = 'w-full px-3 py-2 rounded-lg border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 focus:border-[var(--brand-400)] focus:ring-2 focus:ring-[var(--brand-100)] dark:focus:ring-[var(--brand-900)]/30 outline-none text-[13px] text-slate-900 dark:text-slate-100 placeholder:text-slate-400 dark:placeholder:text-slate-500 dark:text-slate-400 dark:text-slate-500';
...
export const BulkAddModal = ({ isOpen, onClose, onSave }) => {
  ...
  const handleScan = useCallback(async (rawBarcode) => {
    ...
    const parsed = currentMode === 'qr' ? BarcodeParser.parseKarekod(rawBarcode) : BarcodeParser.parse(rawBarcode);
    ...
    setScannedList(prev => [...prev, {
      ...
      name: '',
      quantity: '',
      expiryDate: parsed.formExpiryDate || parsed.expiryDate || '',
      status: 'searching',
    }]);
    setLastScanMsg(null);
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center p-0 sm:p-4" onClick={onClose}>
      ...
      <div
        className="relative bg-white dark:bg-slate-900 w-full max-w-2xl rounded-t-3xl sm:rounded-3xl shadow-2xl border border-slate-200 dark:border-slate-700 max-h-[96vh] flex flex-col animate-[slideUp_.25s_cubic-bezier(.22,.61,.36,1)]"
        onClick={e => e.stopPropagation()}
      >
        {/* Header */}
        <div className="sticky top-0 bg-white/95 dark:bg-slate-900/95 backdrop-blur border-b border-slate-200 dark:border-slate-700 px-6 py-4 flex items-center justify-between shrink-0">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-[17px] font-semibold text-slate-900 dark:text-slate-100 tracking-tight">Toplu İlaç Ekle</h2>
              <p className="text-[12px] text-slate-500 dark:text-slate-400 dark:text-slate-500">Barkod tarayın veya liste yapıştırın</p>
            </div>
          </div>
        ...
        {/* Tabs */}
        <div className="flex border-b border-slate-200 dark:border-slate-700 shrink-0 px-6 pt-3 gap-1">
          {[
            ...
            <button
              ...
              className={`flex items-center gap-1.5 px-4 py-2 text-[13px] font-medium rounded-t-lg transition-colors border-b-2 -mb-px ${
                tab === t.id
                  ? 'border-[var(--brand-600)] text-[var(--brand-700)] bg-[var(--brand-50)] dark:border-[var(--brand-500)] dark:text-[var(--brand-400)] dark:bg-[var(--brand-900)]'
          ...
          <div className="flex flex-col flex-1 overflow-hidden">
              ...
              {!scanning ? (
                <div className="flex gap-3">
                  <button
                    ...
                    className="flex-1 py-2.5 rounded-xl text-[13px] font-semibold transition-all flex items-center justify-center gap-2 border bg-[var(--brand-600)] border-[var(--brand-600)] text-white hover:bg-[var(--brand-700)] shadow-[0_6px_16px_-6px_var(--brand-shadow)]"
                  >
                    <CamIc size={16}/> Karekod (QR)
                  </button>
                  <button
                    onClick={() => { 
                      setScanMode('barcode'); 
                    ...
                    className="flex-1 py-2.5 rounded-xl text-[13px] font-semibold transition-all flex items-center justify-center gap-2 border bg-white dark:bg-slate-900 border-slate-200 dark:border-slate-700 text-slate-700 dark:text-slate-300 hover:bg-slate-50 dark:bg-slate-800 shadow-sm"
                  >
                    <BarcodeIc size={16}/> Barkod
                  </button>
                </div>
              ) : (
                <button
                  onClick={() => setScanning(false)}
                  className="w-full py-2.5 rounded-xl text-[13px] font-semibold transition-all flex items-center justify-center gap-2 border bg-rose-50 border-rose-200 text-rose-700 hover:bg-rose-100"
                >
                  <CamIc size={16}/> {scanMode === 'qr' ? 'Karekod Kamerasını Kapat' : 'Barkod Kamerasını Kapat'}
                </button>
              )}
            ...
            {scanning && (
              <div className="shrink-0 px-6 pb-3">
                <div className="rounded-2xl overflow-hidden border border-slate-200 dark:border-slate-700 shadow-sm relative bg-slate-950" style={{ minHeight: 200 }}>
                  <EmbeddedScanner mode={scanMode} onScan={handleScan} />
                </div>
              ...
              {scannedList.length === 0 ? (
                ...
                <div className="space-y-2">
                  <div className="flex items-center justify-between mb-1">
                    <span className="text-[12px] text-slate-500 dark:text-slate-400 dark:text-slate-500 font-medium">
                      <span className="text-slate-800 dark:text-slate-200 font-semibold">{readyCount}</span> ilaç tarandı
                    </span>
                    {scannedList.length > 0 && (
            ...
            {/* Footer */}
            <div className="sticky bottom-0 bg-white/95 dark:bg-slate-900/95 backdrop-blur border-t border-slate-200 dark:border-slate-700 px-6 py-4 flex justify-end gap-2 shrink-0">
        ...
        {tab === 'text' && (
          <div className="flex flex-col flex-1 overflow-hidden">
            <div className="p-6 flex-1 overflow-y-auto space-y-4">
              <div className="rounded-xl border border-dashed border-slate-300 dark:border-slate-700 bg-slate-50/60 dark:bg-slate-800/60 p-4 text-[12.5px] text-slate-600 dark:text-slate-400 dark:text-slate-500">
                <div className="font-medium text-slate-700 dark:text-slate-300 mb-1.5">Örnek</div>
                <div className="font-mono text-[12px] text-slate-600 dark:text-slate-400 dark:text-slate-500 leading-6 whitespace-pre">
              ...
              />
              <div className="flex items-center justify-between text-[12px] text-slate-500 dark:text-slate-400 dark:text-slate-500">
                <span>
                  <span className="font-semibold text-slate-700 dark:text-slate-300 tabular-nums">{textLines}</span> ilaç hazır
                </span>
...
function EmbeddedScanner({ mode, onScan }) {
  ...
  return (
    <Suspense fallback={
      <div className="flex items-center justify-center py-10 text-slate-400 dark:text-slate-500 text-[13px] gap-2">
        <SpinnerIc size={16} className="animate-spin"/> Kamera yükleniyor…
      </div>
    }>
      <EmbeddedBarcode key={key} mode={mode} onResult={handleResult} />
    </Suspense>
  );
}

// BarcodeScanner'ı gömülü modda kullanan wrapper
function EmbeddedBarcode({ mode, onResult }) {
  return <BarcodeScanner mode={mode} onResult={onResult} onClose={() => {}} embedded />;
}
...
function ScannedRow({ item, index, onRemove, onChange }) {
  ...
  return (
    <div className={`rounded-xl border p-3 transition-all ${
      item.status === 'searching' ? 'border-amber-200 bg-amber-50/40 dark:border-amber-800 dark:bg-amber-900/30' :
      item.status === 'found'     ? 'border-emerald-200 bg-emerald-50/30 dark:border-emerald-800 dark:bg-emerald-900/30' :
      item.status === 'not-found' ? 'border-slate-200 dark:border-slate-700 bg-slate-50 dark:border-slate-700 dark:bg-slate-800/50' :
                                    'border-rose-200 bg-rose-50/30 dark:border-rose-800 dark:bg-rose-900/30'
    }`}>
      <div className="flex items-start gap-2">
        {/* Index */}
        <span className="shrink-0 w-5 h-5 rounded-full bg-slate-200 dark:bg-slate-700 text-slate-600 dark:text-slate-300 text-[11px] font-bold grid place-items-center mt-0.5">
          {index + 1}
        </span>
        ...
        <div className="flex-1 min-w-0 space-y-2">
          ...
          <div className="flex items-center gap-2">
            <button
              ...
              className={`flex items-center gap-1.5 text-[11.5px] font-medium transition-colors shrink-0 ${
                showDate ? 'text-[var(--brand-600)]' : 'text-slate-400 dark:text-slate-500 hover:text-slate-600 dark:text-slate-300'
              }`}
            >
              <CalIc size={12}/>
          ...
          {/* Barcode chip */}
          {item.barcode && (
            <span className="inline-flex items-center gap-1 text-[10.5px] font-mono text-slate-400 dark:text-slate-500 bg-slate-100 dark:bg-slate-800 px-2 py-0.5 rounded-full">
        ...
        <button onClick={onRemove}
          className="shrink-0 p-1.5 rounded-lg text-slate-400 dark:text-slate-500 hover:text-rose-600 hover:bg-rose-50 transition-colors mt-0.5">
          <TrashIc size={13}/>
        </button>
      </div>
    </div>
