...
const PencilIc = (p) => <Ic {...p} extra={<><path d="M17 3a2.85 2.83 0 1 1 4 4L7.5 20.5 2 22l1.5-5.5Z"/></>}/>;
...
export function FamilyModal({ user, onClose, onFamilyChange }) {
  ...
  const handleRenameFamily = async (e) => {
    ...
    setBusy(true); setError('');
    try {
      const newName = await FamilyService.renameFamily(family.id, renameValue);
      setFamily(prev => prev ? { ...prev, name: newName } : prev);
      setRenamingFamily(false);
      onFamilyChange?.();
      setSuccess('Aile adı güncellendi.');
    } catch (e) { setError(e.message); }
    finally { setBusy(false); }
  };
  ...
  const myRole = family?.members?.[user.uid]?.role;
  const isAdmin = myRole === 'admin';
  const canEditFamily = myRole === 'admin' || myRole === 'editor';
        ...
        <div className="flex items-center justify-between px-5 py-4 border-b border-slate-200 dark:border-slate-700 shrink-0">
          <div className="flex items-center gap-2.5">
            ...
            <div>
              ...
              {view === 'main' && family && renamingFamily ? (
                <form onSubmit={handleRenameFamily} className="flex items-center gap-1 mt-0.5">
                  <input value={renameValue} onChange={e => setRenameValue(e.target.value)}
                    maxLength={60} autoFocus aria-label="Aile adı"
                    className="px-2 py-1 rounded-lg border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 text-[12px] text-slate-900 dark:text-slate-100 outline-none focus:border-[var(--brand-500)] w-36"/>
                  <button type="submit" disabled={busy || !renameValue.trim()} aria-label="Kaydet"
                    className="p-1 rounded-md text-emerald-600 hover:bg-emerald-50 dark:hover:bg-emerald-950/30 disabled:opacity-40">
                    <CheckIc size={13}/>
                  </button>
                  <button type="button" onClick={() => setRenamingFamily(false)} aria-label="Vazgeç"
                    className="p-1 rounded-md text-slate-400 dark:text-slate-500 hover:bg-slate-100 dark:hover:bg-slate-800">
                    <XIcon size={12}/>
                  </button>
                </form>
              ) : (
                <div className="text-[11px] text-slate-500 dark:text-slate-400 dark:text-slate-500 flex items-center gap-1">
                  <span>{family ? family.name : 'İlaçlarınızı aile üyeleriyle paylaşın'}</span>
                  {view === 'main' && family && canEditFamily && (
                    <button type="button" onClick={() => { setRenameValue(family.name); setRenamingFamily(true); setError(''); setSuccess(''); }}
                      aria-label="Aile adını düzenle" title="Aile adını düzenle"
                      className="p-0.5 rounded text-slate-400 dark:text-slate-500 hover:text-[var(--brand-600)] hover:bg-slate-100 dark:hover:bg-slate-800 transition-colors">
                      <PencilIc size={11}/>
                    </button>
                  )}
                </div>
