...
export const DeleteModal = ({ medicine, onClose, onConfirm }) => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center p-0 sm:p-4" onClick={onClose}>
      ...
      <div
        className="relative bg-white w-full max-w-md rounded-t-3xl sm:rounded-3xl shadow-2xl border border-slate-200 animate-[slideUp_.25s_cubic-bezier(.22,.61,.36,1)]"
        ...
        <div className="p-6">
          <div className="flex items-start gap-3">
            ...
            <div className="flex-1">
              <h2 className="text-[17px] font-semibold text-slate-900 tracking-tight">İlacı sil?</h2>
          ...
          {total > 1 && (
            <div className="mt-4 rounded-xl border border-slate-200 bg-slate-50/60 p-4">
              <div className="text-[12px] font-medium text-slate-700 mb-2">Kaç adet silinsin? (toplam {total})</div>
              <div className="flex items-center gap-3">
                ...
                />
                <span className="tabular-nums text-[18px] font-semibold text-slate-900 w-10 text-center">{n}</span>
              </div>
            </div>
          )}
        </div>
